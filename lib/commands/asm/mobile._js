...
exports.init = function (cli) {
  ...
  function createProxyFilter() {
    var proxyUrl = loadEnvironmentProxyValue();
    if (proxyUrl) {
      log.silly('Proxy: ' + proxyUrl);
      var parsedUrl = url.parse(proxyUrl);
      if (!parsedUrl.port) {
        parsedUrl.port = 80;
      }

      var filter = azureCommon.ProxyFilter.create({
        host: parsedUrl.hostname,
        port: parsedUrl.port,
        protocol: parsedUrl.protocol
      }, parsedUrl.protocol.toLowerCase() === 'https:');

      var filterWrapper = function (resource, next, callback) {
        resource.url = resource.uri;
        return filter(resource, next, callback);
      };

      return filterWrapper;
    }
    log.silly('No proxy defined.  Please set the environment variable HTTPS_PROXY to your proxy url');
    return undefined;
  }
  ...
  function createClient(options) {
    var currentSubscription = profile.current.getSubscription(options.subscription),
        client = azureCommon.requestPipeline.create(
          ...
          utils.createFollowRedirectFilter()),
        proxyFilter = createProxyFilter();

    if(proxyFilter !== undefined){
      client = azureCommon.requestPipeline.createWithSink(client, proxyFilter);
    }
  ...
  */
  ...
  mobile.trackAsyncOperation = function (options, requestId, callback) {
    function waitOne(errorCounter) {
      ...
      var asyncChannel = new PipelineChannel(client, webResource, log)
        .path('operations')
        .path(requestId)
        .header('Accept', 'application/json');
